<#
make_release.ps1 -- build the Windows release zip for DuckHunt.

Ships ONE windows zip (never a bare exe -- the exe needs SDL2.dll and the
launcher/ assets):

  DuckHuntRecomp-windows-x64.zip
      DuckHuntRecomp.exe + SDL2.dll + keybinds.ini + launcher/ + launcher-deps/ + README.txt
...
Publish AFTER smoke-testing the zip from a scratch directory:
  ...
  gh release create vX.Y.Z release\DuckHuntRecomp-windows-x64.zip `
...
$exe = Join-Path $bin 'DuckHuntRecomp.exe'
if (-not (Test-Path $exe)) { throw "missing $exe -- build the game first (build_all.bat, or cmake --build build_release)" }

$readme = @'
Duck Hunt - Static Recompilation
================================

A native PC build of Duck Hunt, produced by statically recompiling the NES
ROM's 6502 code to C with the NESRecomp framework (github.com/mstan/nesrecomp).

No ROM is included. On first launch, select your legally-obtained Duck Hunt
...
$zip = Join-Path $out 'DuckHuntRecomp-windows-x64.zip'
